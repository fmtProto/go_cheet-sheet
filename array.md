...
arr5 := [...]int{}              // [] пустой массив
arr6 := [0]int{}                // [] пустой массив
```
...
Получить значение определенного элемента массива, можно обратившись к нему по номеру индекса.   
Индексы находятся в диапазоне `0` по `len(array) - 1`.  
Операция имеет  сложность `O(1)`.
```go
arr := [3]int{1, 2, 3}
fmt.Println(arr[1]) // 2
```
...
В Go размер массива(длина) является частью его типа. Это означает, что два массива с разным количеством элементов относятся к двум разным типам. Длина массива фиксируется при создании и не может быть изменена позже, именно потому что, она является частью типа.
```go
arr1 := [3]int{1, 2, 3}
arr2 := [2]int{1, 2}
...
Так же стоит отметить, что функцию `cap()` показывающую вместимость в слайсах, можно использовать и с массивами, это не вызовет ошибку, однако использовать ее не идиоматически. В массивах результат `cap()` всегда равен результату `len()`.
```go
arr := [3]int{1, 2, 3}
