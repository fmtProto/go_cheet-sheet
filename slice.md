...
Так как срез является ссылочным типом данных, то при присвоении просто создается еще одна переменная, указывающая на ту же область памяти, что и исходный срез. Поэтому любые изменения в одном из этих срезов отразятся на другом, кроме конечно действий связанных с аллокацией памяти, например, при использовании `append()`
```go
s1 := []int{1, 2, 3}
s2 := s1
fmt.Println(s1, s2)           // [1 2 3] [1 2 3]
fmt.Printf("%p - %p", s1, s2) // 0xc0000a4000 - 0xc0000a4000

s1[0] = 444
s2[1] = 555
fmt.Println(s1, s2) // [444 555 3] [444 555 3]

s1 = append(s1, 4, 5, 6)
fmt.Println(s1, s2)           // [444 555 3 4 5 6] [444 555 3]
fmt.Printf("%p - %p", s1, s2) // 0xc0000ac000 - 0xc0000a4000
// срезы теперь ссылаются на разные адрессные пространства
// и изменения в них независимы
```

При помощи `copy()` можно создать копию среза, при этом функция скопирует элементы из исходного среза в срез назначения и вернет количество скопированных элементов. Срез назначения должен быть длины не меньше чем исходный срез.
```go
// синтаксис функции
...
fmt.Println(src, dst) // [a b c] [a b c]
fmt.Printf("%p - %p", src, dst) // 0xc00008c150 - 0xc00008c180
```
При помощи `append()`.
```go
src := []string{"a", "b", "c"}
...
fmt.Println(src, dst) // [a b c] [a b c]
fmt.Printf("%p - %p", src, dst) // 0xc0000a2150 - 0xc0000a2180
```
